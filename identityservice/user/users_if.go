...
func UsersInterfaceRoutes(r *mux.Router, i UsersInterface) {
	...
	r.Handle("/users/{username}/see/{globalid}", alice.New(newUserIndentifierMiddleware().Handler, newOauth2oauth_2_0Middleware([]string{"user:admin"}).Handler).Then(http.HandlerFunc(i.GetSeeObjectsByOrganization))).Methods("GET")
	r.Handle("/users/{username}/see/{globalid}/{uniqueid}", alice.New(newUserIndentifierMiddleware().Handler, newOauth2oauth_2_0Middleware([]string{"user:admin"}).Handler).Then(http.HandlerFunc(i.GetSeeObject))).Methods("GET")
